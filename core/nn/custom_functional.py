...
def gradient_central_diff(input):
    return input, input
    kernel = [[1, 0, -1]]
    kernel_t = 0.5 * torch.Tensor(kernel).to(input.device) * -1.  # pytorch implements correlation instead of conv
...
def numerical_gradients_2d(input):
    """
    ...
    n, c, h, w = input.shape
    assert h > 1 and w > 1
    x, y = gradient_central_diff(input)
    return x, y
...
def convTri(input, r):
    ...
    kernel = torch.Tensor([f]).to(input.device) / (r + 1) ** 2
...
def compute_normal(E):
    if torch.sum(torch.isnan(E)) != 0:
        print('nans found here')
        import ipdb;
        ipdb.set_trace()
    E_ = convTri(E, 4, cuda)
    Ox, Oy = numerical_gradients_2d(E_)
    Oxx, _ = numerical_gradients_2d(Ox)
    Oxy, Oyy = numerical_gradients_2d(Oy)
